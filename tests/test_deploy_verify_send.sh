...
grep -q 'cw_deploy_build_verify_prompt' ../bin/deploy-verify-send.sh \
  || { echo "FAIL: missing verify-prompt builder" >&2; exit 1; }
grep -q 'verify-cody-' ../bin/deploy-verify-send.sh \
  || { echo "FAIL: missing per-round filename" >&2; exit 1; }
grep -q 'verify-report-' ../bin/deploy-verify-send.sh \
  || { echo "FAIL: missing report filename" >&2; exit 1; }
grep -q 'test-output-' ../bin/deploy-verify-send.sh \
