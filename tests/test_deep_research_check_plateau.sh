#!/usr/bin/env bash
# tests/test_deep_research_check_plateau.sh — 5-exp <1% stagnation logic
set -euo pipefail
cd "$(dirname "$0")"
...
if cw_deep_research_check_plateau "$SB" "$TMP/cursor1.txt"; then
...
if ! cw_deep_research_check_plateau "$SB" "$TMP/cursor2.txt"; then
...
if cw_deep_research_check_plateau "$SB" "$TMP/cursor3.txt"; then
...
if cw_deep_research_check_plateau "$SB" "$TMP/cursor4.txt"; then
...
if ! cw_deep_research_check_plateau "$SB" "$TMP/cursor5.txt"; then
...
write_sb "$SB" "exp-001:0.99:ok" "exp-002:0.989:ok" "exp-003:0.991:ok" \
               "exp-004:0.988:ok" "exp-005:0.990:ok"
if ! cw_deep_research_check_plateau "$SB" "$TMP/nope.txt"; then
  echo "FAIL: rc!=0 when cursor file missing" >&2; exit 1
fi
pass "missing cursor file — treated as 0"

echo "test_deep_research_check_plateau: 6 assertions green"
