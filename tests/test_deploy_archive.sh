...
mkdir -p "$TD/_deploy"
echo "design" > "$TD/_deploy/design.md"
echo "plan"   > "$TD/_deploy/plan.md"

../bin/deploy-archive.sh "$TOPIC"

ARCHIVE_BASE="$CLONE_WARS_HOME/archive/$RH/$TOPIC"
[[ -d "$ARCHIVE_BASE" ]] || { echo "FAIL: archive base missing" >&2; exit 1; }
n=$(ls "$ARCHIVE_BASE" | grep -c '^_deploy-' || true)
[[ "$n" -eq 1 ]] || { echo "FAIL: expected exactly one _deploy-* dir, got $n" >&2; exit 1; }
[[ ! -d "$TD/_deploy" ]] || { echo "FAIL: source _deploy/ still present" >&2; exit 1; }
pass "archive moves _deploy → archive/_deploy-<ts>"

# Refuses if _deploy/ missing.
err=$(../bin/deploy-archive.sh "$TOPIC" 2>&1) && rc=0 || rc=$?
...
mkdir -p "$TD/_deploy"
echo "rerun" > "$TD/_deploy/design.md"
# Pre-create today's expected target so the first archive will collide.
TS=$(date -u +'%Y%m%dT%H%M%SZ')
mkdir -p "$ARCHIVE_BASE/_deploy-$TS"
../bin/deploy-archive.sh "$TOPIC"
# Now there should be at least 2 _deploy-* dirs (the pre-created one + the -2 suffix).
n2=$(ls "$ARCHIVE_BASE" | grep -c '^_deploy-' || true)
[[ "$n2" -ge 2 ]] || { echo "FAIL: collision counter didn't fire; got $n2 dirs" >&2; exit 1; }
ls "$ARCHIVE_BASE" | grep -q '^_deploy-.*-2$' \
