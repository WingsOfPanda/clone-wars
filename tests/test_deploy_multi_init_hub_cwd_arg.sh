...
# v0.31.0: invoke from HUB_DIR (the conductor's $PWD in the real flow) +
# pass HUB_DIR as 2nd arg. cw_deploy_art_dir uses $PWD-keyed hash, which
# matches the test's pre-populated path (HUB_DIR's repo-hash). The
# v0.30.0 CW_TOPIC_REPO_CWD env var is dead in v0.31.0.
( cd "$HUB_DIR" && CLAUDE_PLUGIN_ROOT="$PLUGIN_ROOT" CLONE_WARS_HOME="$CLONE_WARS_HOME" \
