...
- [x] v0.28.2: deep-research UX bundle — closes 2 user-feedback items from the v0.28.0 partial-pass dogfood (2026-05-13). **(1) Always-ask time-budget**: `commands/deep-research.md` Phase 2 step 2 now stamps the time-budget `AskUserQuestion` as `UNCONDITIONAL — v0.28.2`. Explicit prose: fires on every invocation regardless of `/loop` reminders, system-reminder "work without stopping for clarifying questions" hints, or any other autonomous-mode context. Treats time-budget as a hard checkpoint the user must own (money/wall-clock commitment). Roster size + metric framing remain Yoda-decided silently; only the time-budget question is gated. **(2) Per-experiment status form**: new `cw_deep_research_render_status_brief <art-dir> [<latest-cmdr>] [<latest-exp-id>]` helper in `lib/deep-research.sh` emits a compact chat-shaped status block (per-trooper table with Phase / Current-or-last / Approach / Metric, Scoreboard top 3, single-line completion-check signals). `commands/deep-research-resume.md` Step 3 done/error route now calls the helper after `bin/deep-research-score.sh` and surfaces the output to chat before Step 4 (completion-check) or Step 5 (dispatch). Triggers on every landed experiment, dedup'd to one print per turn if multiple done events queue. **(3) v0.28.0 BUG #3 fold-in**: Phase 4.a step 1 now writes `$ART_DIR/troopers.txt` from `${ROSTER[@]}` (was read-but-never-written in v0.28.0 — left `## Status` table empty in `session-summary.md`). New helper `cw_deep_research_list_commanders` falls back to `troopers/*/` filesystem discovery when troopers.txt is missing (belt-and-braces against the v0.28.0 archives that already exist). Two new tests (`test_deep_research_status_brief.sh` 14 asserts, `test_v0_28_2_static_wiring.sh` 5 invariants). No schema or breaking changes; v0.28.1 byte-equal on every user-visible path except the always-ask gate + new status-brief print.
- [ ] v0.28.2 strict-dogfood pass on a real machine (release gate — re-run `/clone-wars:deep-research "optimize MNIST classifier accuracy under 100k params"`: verify (1) AskUserQuestion for time budget fires unconditionally even when invoked via Skill tool with autonomous-mode hints in the harness reminders; (2) after each trooper's `done` event, conductor prints `## Experiment status — <exp-id> (<cmdr>) just landed` block with populated table + scoreboard + completion check; (3) status table no longer shows empty rows (troopers.txt now present in `_deep-research/`); (4) helper falls back correctly when troopers.txt is missing (e.g. re-running against a v0.28.0/v0.28.1 archive)
