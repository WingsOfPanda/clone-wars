...
- [ ] v0.28.0 strict-dogfood pass on a real machine (release gate — re-run `/clone-wars:deep-research "optimize MNIST classifier accuracy under 100k params"`: verify (1) Phase 4.a enters cleanly, N Monitor tasks visible in TaskList, first dispatch round fires + Yoda's turn ends; (2) trooper `done` event fires Yoda turn within seconds of `result.json` write via `<task-notification>`; (3) mtime stale → Yoda sends `status?` probe via `bin/send.sh`; trooper response clears it; phase returns to `working`; (4) mtime stuck after probe → Yoda surfaces decision to user or aborts per judgment; (5) user types "stop research" mid-session → halt.flag touched → finalize.sh runs → Phase 5 synthesis fires; (6) `session-summary.md` readable + accurate at any point during the run (Status / Scoreboard top 5 / Completion check / Recent events sections all live); (7) UserPromptSubmit hook injects handler 3.b context on chat turns during active session; cheap no-op when no session active; (8) after teardown, hook stops injecting (active.txt removed); (9) completion-check signals correctly fire stop when target + K corroboration both satisfied (Yoda's judgmental override respected); (10) Yoda dispatches "directions" of 1-2 sentences not detailed plans — trooper autonomy preserved)
