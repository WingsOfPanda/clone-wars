#!/usr/bin/env bash
# bin/deploy-plan-send.sh — Phase 1 plan dispatch (codex).
#
# Usage: bin/deploy-plan-send.sh <topic>
#
# Writes _deploy/plan-cody.txt with one line: OFFSET=<n>.
...
cw_deploy_build_plan_prompt "$DESIGN" "$PLAN_OUT" > "$PROMPT_FILE"
