...
the Jedi general crashes; you can `tmux select-pane` mid-task and see exactly what each model
is doing.
...
| `/clone-wars:collect <commander> <topic> [--timeout s]` | Block until the trooper reports `done` or `error`, then print the summary. Exits non-zero on error/timeout so the Jedi general can chain commands. |
...
investigation command. The slash directive walks the Jedi general through
...
8. `consult-adjudicate` writes `adjudicated-draft.md`. the Jedi general copies
...
Between every step the Jedi general regains control: if a trooper writes
malformed findings, the Jedi general can `cw_send` a clarifying prompt,
